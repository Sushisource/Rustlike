...
pub trait ContextHelp {
  ...
  fn center_rect(&mut self, center: Point, w: Meters, h: Meters) -> GameResult<()>;
...
impl ContextHelp for Context {
  ...
  fn center_rect(&mut self, c: Point, w: Meters, h: Meters) -> GameResult<()> {
    let r = Rect { x: c.coords.x - w / 2.0, y: c.coords.y - h / 2.0, w, h };
    let r = Mesh::new_rectangle(self, DrawMode::fill(), r, Color::new(1.0, 1.0, 1.0, 1.0))?;
